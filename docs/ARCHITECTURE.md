...
Codelabz is a collaborative tutorial platform where developers can write, publish, and co-edit step-by-step technical tutorials. This document explains how the project is structured, what technology choices were made, and why.
...
Firebase provides auth, database, storage, and serverless functions in one platform. For a project of this scale, it eliminates the need to maintain a separate API server, handle deployments, or manage infrastructure.

**Why two databases (Firestore + Realtime DB)?**
Firestore handles structured document queries well — user profiles, tutorial metadata, organization settings. Firepad (the collaborative editor library) requires Firebase Realtime DB specifically for its low-latency synchronization protocol. So both are used intentionally for what they do best.

**Why Redux alongside react-redux-firebase?**
react-redux-firebase automatically syncs Firebase auth and Firestore data into Redux. Custom reducers (auth, org, tutorials, etc.) handle UI-specific state that doesn't map cleanly to Firestore documents — like form loading states, editor cursor positions, or error messages.

**Why Elasticlunr for search?**
There is no Algolia or dedicated search backend. Tutorials are indexed client-side on app load using Elasticlunr, a lightweight full-text search library. This keeps the architecture simple at the cost of scale — it works well for a tutorial library of moderate size.
