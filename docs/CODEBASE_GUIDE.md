...
This document is a map of the repository. It explains what every major folder and file does so you can navigate the codebase confidently without reading everything first.
...
### `src/config/`
Firebase initialization lives here. `index.js` reads environment variables, builds the `firebaseConfig` object, initializes all Firebase services, and conditionally points them at the local emulator when `VITE_APP_USE_EMULATOR=true`. If Firebase isn't connecting, this is the first file to check.
...
| `Tutorials/` | Tutorial browse, search, and card grid |
...
Firebase Cloud Functions. These run server-side on Google's infrastructure — not on any machine you manage.
...
Individual components can be developed and documented in isolation using Storybook. Stories live in `src/stories/`. Run with `npm run storybook`.
